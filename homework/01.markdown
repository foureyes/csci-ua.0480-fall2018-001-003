...
If the "letter" is empty, use a space character to space out the cell. If the "letter" is more than one character, truncate it to fit in the space.
...
Use a previous function that you created, `toIndex` in the implementation of this function. Again, you can __assume that the space character, `" "`, represents empty__.
...
<pre><code data-trim contenteditable>const computerMoves = process.argv[2]
</code></pre>
...
1. At the beginning of your file, use `process.argv` to access the passed in argument... and use `JSON.parse` to turn it into an actual nested `Array`
    <pre><code data-trim contenteditable>const computerMoves = process.argv[2] ? JSON.parse(process.argv[2]) : undefined;
</code></pre>
