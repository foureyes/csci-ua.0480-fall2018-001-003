...
This will return 5 as an integer when you use `parseFloat(obj.overall_satisfaction)`.
...
1. When creating your report in `processBiteData`, you must use all of the following Array methods at least once each in your program:
    ...
    * HINT: A typical pattern is to always use map 99% of the time, so you never accidentaly get rid or mess up the data, except when you're doing your first pass through the data to clean it up. You don't an initial copy of the data with all the numbers saved as strings and weird typos etc, so forEach is good for that.
