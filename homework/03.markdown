...
1. using the sample code from class, refactor the code into functions and create a website about an animal (maybe your pet, your favorite animal, your _spirit animal_, a mythical animal, etc.)
2. refactor your code again, but move the functionality into reusable classes that mimic Express routes and middleware (specifically `.get` and `express.static` middleware

At the end, you'll have a reusable _toy_ web framework that you can use to write simple web applications.
...
__Push__ you code to the homework repository on GitHub. Repositories will close, so make sure you push your changes before the deadline.
...
1. `src/webby1.js` - a module that contains helper functions and code to create a tcp/ip server (with http layered on top of it) 
2. `src/server1.js` - a file that uses `webby1.js` to start the server
...
1. `GET /` - responds with a page that contains a links to `/links` (on this same site)
2. `GET /css/styles.css` - responds with a stylesheet
3. `GET /links` - responds with a page that has links to resources about your favorite animal
4. `GET /img/animal.jpg` - responds with an image of your favorite animal
...
Follow these steps to implement part 1:

1. Start by creating a module containing helper functions and objects to get a simple web server working.
2. Add a mechanism to serve images
3. Build out the iste described above


### 1. Create a Simple Web Server

Start by creating some functions/objects in `webby1.js` for helpers, utilities, and an entry point to creating a web server. __No type checking or error handling has to be handled unless specified by the description below__. You'll export some of these for use in `server1.js`. Minimally, implement the following (of course, you can create more functions/objects, as the ones listed below will only help with a subset of the part 1's requirements): 


### `HTTP_STATUS_CODE_DESCRIPTIONS`

Create a module-level (_global_) object called `HTTP_STATUS_CODE_DESCRIPTIONS` that maps status codes (as `Number`s) to descriptions. For example, `200` should map to `OK`. Minimally, this should contain 200, 404, and 500.
...
### `createResponseHead(status, contentType)`

__Parameters:__

* `status` - a `Number` representing the status code of an HTTP response 
* `contentType` - a `String` representing the value for the `Content-Type` header for an HTTP response  (see `getMIMEType` above for possible values)

__Return:__

* a `String` that contains the status line and the `Content-Type` header, along with the blank line (exactly two `\r\n` after the `Content-Type` header)

__Description:__

This function constructs the "top" part of an HTTP response. It includes only the status line and a single header, `Content-Type`. Hint: Use the `HTTP_STATUS_CODE_DESCRIPTIONS` object created previously to map codes to short descriptions.

Example usage:

```
const responseHead = createResponseHead(200, 'text/html');
// will give back the string below (note the extra new line at the end)
```
```
HTTP/1.1 200 OK
...
### `createHTML(title, css, content)`
 
__Parameters:__

* `title` - the text in the title tag of the resulting html
* `css` - the path to the css file to be linked to in the resulting html
* `content` - the tags / content in the body tags of the resulting html file

__Return:__

* a `String` representation of an html document generated using the arguments passed in

__Description:__

```
const html = createHTML('foo', '/bar/baz.css', '<h1>qux</h1>');
// html should be a string with the following content...
```
...
<head>
<title>foo</title>
<link rel="stylesheet" href="/bar/baz.css">
</head>
<body>
	<h1>qux</h1>
</body>
...
1. creates a server...
2. in such a way that sets the callback called when a client connects to `handleConnection` (see function specification for `handleConnection` down below)
3. listens on the port and host

Hint: this should be a very short function! ... use the `net` module to create a server and listen with that server. The callback should be `handleConnection`, specified below.
...
* `sock` - a socket object that represents the connected client
...
Once a client connects (remember, this is called on connection), _find_ a way to make it so that when the server receives data, the `handleRequest` function below is called. You'll notice this is a bit tricky, though, since the method on the `sock` that sets the callbacke
sock.on('data', binaryData => handleRequest(sock, binaryData));
...
const [method, path, ...rest] = reqStr.split(' ');
if(path === '/') {
  const s = createHTML('Geckos', 
    '/css/style.css', 
    `<h1>Geckos R Great!</h1>
    <a href="/links">Links about geckos</a>`
  );
  sock.write(createResponseHead(200, 'text/html'));
  sock.write(s);
  sock.end();
} else if(path === '/links') {
  const s = createHTML('Geckos', 
  ...
  const s = `
    body { background-size: 194px 130px; background-image: url(/img/animal.jpg); background-repeat: repeat;}
  `;
  sock.write(createResponseHead(200, 'text/css'));
  sock.write(s);
  sock.end();
} else if(path === '/img/animal.jpg') {
  sendImage(sock, 'public', 'img/animal.jpg');
} else {
  sock.write(createResponseHead(404, 'text/plain'));
...
### `handleRead(sock, contentType, err, data)`:
...
if(err) {
  console.log(err);
  sock.write(createResponseHead(500, 'text/plain'));
} else {
  sock.write(createResponseHead(200, contentType));
  sock.write(data);
}
sock.end();
