...
# A Site About An Animal 🐶🦎🦉🐬🐷, Due Friday, Oct 5th at 11PM 
...
5. __you'll have to create your own `package.json`, `package-lock.json`, `.gitignore`__
    * (these are required and included in grading)
...
In this part, you'll use the [sample code](../#class7) from our in-class demos on the `net` module to implement classes that mimic the `Express` framework (of course, you can't use `Express` for this). Your framework (`webby.js`), will be built off of node's `net` module. It will use the `net` module to create a TCP server that will allow connections from clients. It write will handle an incoming http request from a client by parsing the http request, determining what do based on the request, and finally sending back an http response.
...
* instead of relying on functions and a single script, you'll create a module that contains classes, funtions and objects that can be brought in (using `require`) by another script:
    ...
    7. `App` - a class that represents a web application; takes incoming requests and determines what to do based on eisting middlesware, path, method, etc. ... 
    8. `serveStatic` (exported as `static`) - a middleware function that serves a file if it exists on the file stystem
* the classes and objects provided by the framework will have several features that we did not implement in class:
    * can add routes without needing to have knowledge about the routing implementation (that is, without having to add to a large conditional)
    * can add middleware that gets called before routes
    * is able to handle paths with different casing, fragments, query strings, etc.
    * can send files from the file system without defining explicit route
...
3. go over the [notes from previous classes](../examples/class06/notes.js)
4. lastly, make sure that you can write back a valid http response by reviewing::
    * [the slides on http](../slides/05/web.html#/16)
    * [and an example response](../slides/05/web.html#/24)

__No type checking or error handling has to be done unless specified by the description below / the unit tests provided__. You'll export some of these for use in `server1.js`. Minimally, implement the following (of course, you can create more functions/objects, as the ones listed below will only help with a subset of the part 1's requirements): 
...
3. create an  object that encapsulates your server...
...
In your module, `webby.js`, create the classes and objects specified below. You can periodically run the tests in `test/webby-test.js` to ensure that your implementations are correct.
...
Create a module-level (_global_) object called `HTTP_STATUSS` that maps status codes (as `Num er`s) to descriptions. For example, `200` should map to `OK`. Minimally, this should contain `200`, `404`, and `500`.
...
* the [MIME type](https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/MIME_types) of a file based on its extension
    ...
    * (see article above for how appropriate MIME type for extension)
...
const t2 = getExtension('foo'); // t2 --> '' (empty string)
```
...
Create the `Request` _class_  by implementing the following in `webby.js`:
...
Once your finished with your implementation above, you can export, test and try using your new objects and `Request` class.

1. Export the object using `module.exports`, and try running the tests in `test/webby-test.js` (use [`.only`](https://mochajs.org/#exclusive-tests) for each object you want to test or simply comment out the other tests) ... run from within `root` folder
...
3. As a consequence, you'll be able to use `Request` instance's `method` and `path` properties  to conditionally execute code     
...
3. optionally calling a middleware function if it exists
...
3. `middlware` - a function that gets called before the functions in `routes`... which enables the user to add features that will be executed before every route specified
    * useful for... adding logging
    * or serving static files (see `serveStatic` later on in these specifications)
...
3. `get(path, cb)` - adds `GET` and `path` to "key" into `routes`... the value of which is the callback function, `cb` 
...
4. `use(cb)` - sets the `middleware` property for this instance of `App`
    * `cb` - the function called before every route handler in `routes` 
        ...
        * a `Request` object
        * a `Response` object
        * a `next` function - representing the next function to call after the middeware is done processing the `Request` (which basically allows the processing af a `Request` to optionally continue through the route handlers)
    * no return value
    * example usage:
...
6. `handleConnection(sock)` - the function called when a client connects to the server... this will simply set the callback for the socket's `on` method: `sock.on('data', ...)` to the function below, `handleRequest`
    ...
    * no return value
    * ⚠️when you set the callback function to `sock.on` to `handleRequest`, you'll have to find a way to turn `handleRequest` (see below) into a function that takes one argument instead of two ([the callback to `sock.on('data', ...)` is only passed a `String` or `Buffer`, not a `Socket`]!)
...
4. `handleRequest(sock, binaryData)` - the function called when the socket receives data from the client (our framework will not have a timeout, it'll just assume that once it receives data, that the data received is the entire request)...  this is where your app will create `Request` and `Response` objects and determine whether or not it should call the `middleware` function 
    ...
    * this function is responsible for:
        ...
        2. calling the `middleware` function if it's been set (not `null`)
            ...
            4. ⚠️ note that in this case, care must be taken with the way that `processRoutes` is passed in so that its this `value` is the `App` instance 
        2. if there's no `middleware` function see if the method and path matches any of the defined routes in the `App` object's `routes` property by calling `processRoutes` (specified below) with the `Request` and `Response` objects created earlier in this method
5. `processRoutes(req, res)` - calls the appropriate function stored in `routes` to handle the incoming request based on `method` and `path`
    ...
    * no return value
    * this callback is essentially responsible for determining if the requeset is supported (based on `method` and `path` and sending back a response... it will:
        1. look up the function to call in `this.routes` by using the `method` and `path` property from the incoming `Request` object ... Hint: __use a method defined earlier to help deal with casing, traling paths, etc.__
        2. call that function, passing in the `Request` and `Response` objects that were passed in to this function as arguments 
...
`Response(socket, statusCode, version)` - creates a new response object using the socket passed in as an argument to send data to the client. `version` and `statusCode` can be used to initialize the `version` and `statusCode` properties (specified below). If `version` and `statusCode` are not passed in, the default value for `version` should be `"HTTP/1.1"` and `200` for `statusCode`.
...
3. `end()` - sends data and ends the connection by callings the `end` method on this `Response` object's internal socket object (essentially a pass-through / proxy method to call the same method on `this.sock`)
    ...
    * also note that you should not call `sock.write` at all in this method; again, just make the same call to `sock.end` 
    * no return value
    * example usage:
...
3. `headersToString()` 
    * no parameters
    * returns a `String` representing the headers of this http response (the both the name and the value), with each header name/value pair ending with `"\r\n"`
    * HINT: use one of the function you've already defined to add the description of the status code
    * example usage:
...
4. `send(body)` - sets the `body` property of this `Response` object. Sends a valid http response to the client based on this `Response` object's properties (`statusCode`, `version`, and `headers`) and the `body` argument, and closes the connection. It will set the `Content-Type` header to `text/html` even if the header doesn't exist in `headers`. ⚠️It should write the status line and headers to the socket and the body (note that there should be an extra `"\r\n"` that should be sent out before the body to separate the body from what was written out before.
    * `body` - the body of the http response
    * no return value
    * HINT: ⚠️again, write out the status line (first line), headers, and extra new line before writing out the body (use this `Response` object's `sock` property to do this)
    * example usage:
...
2. If everything the tests for `App`, `Response` (and the other previously defined functions and class), you can modify `app.js` to create a simple web application (which you'll replace later):
    <pre><code data-trim contenteditable>const webby = require('./webby.js');
const app = new webby.App();
...
This new middleware function will attempt to read a file from disk based on the request path. If it exists, the file will be served by sending back an appropriate http response (and the `routes` defined in the `App` object will be skipped). If it doesn't exist, then the rest of the routes will tried.
...
* it returns a new function
    ...
    * put together the path on the file system to attempt to read the file from by using the `path` module and `path.join` (see the example code) on the `basePath` and the path from the `Request` object, `req.path`; use the two argument version of `fs.readFile` to do this (⚠️ only pass in the path and the the callback, don't specify options or encoding!)
        * for example, if `serveStatic` were called with `/projects/myname-homework03/public` and the incoming http request contained `/css/styles.css`, the resulting middleware function returned from `serveStatic` will attempt to read `/projects/myname-homework03/public/css/styles.css`
        * if the reading the file is successful, use the `Response` object passed in to send back the contents of the file that was read
        * you'll have to find a way to set the correct status and the right `Content-Type` in your response; it's adequate to use the extension of the file read to do this
    * if the file cannot be read or if _any_ error occurs (it's adequate to check the `err` argument passed into the `fs.readFile` callback), then call the `next` function passed in with the `Request` and `Response` objects passed in
        * this `next` function is likely going to be `processRoutes` from the `App` object above (remember, you have to be ⚠️ careful about making sure `this` is properly set)
...
Choose an animal to create a site about. It should respond to the following requests:
