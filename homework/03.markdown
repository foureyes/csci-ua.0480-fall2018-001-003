...
Start by creating some functions/objects in `webby1.js` for helpers, utilities, and an entry point to creating a web server. You'll export some of these for use in `server1.js`. Minimally, implement the following (of course, you can create more functions/objects, as the ones listed below will only help with a subset of the part 1's requirements). __No type checking or error handling has to be handled unless specified by the description below__.
...
console.log('ctrl + c if u dont want me 2 serve nemore');
