...
# Forest Simulator (Client Side JavaScript) - __Due {HW_07_DUEDATE}__
...
* Note that there's a form field in the _title screen_ ...
    * This field will allow the player to manuall set the emoji forest in the sim (it's kind of like cheating / playing Intelligent Creator, but it's really for making it easier to test)
...
* Create a function or object that generates an 8x8 forest
    * You'll use this any time a forest is needed for the player or the computer
...
* Here's an example of how it may work (again, the sim screen_ will be blank for now, but when it's implemented, it should function like this):
    <img src="../resources/img/hw07-forest-03-genagain.gif">
    <br>


__Generate DOM elements for the lines of forest, the biodiversity emtric, and 3 buttons__
...
__Selecting rows to pin__
* add onclick handlers to all of the rows so that when clicked, it toggled between a pinned and unpined state
...
* Add an onclick handler to the generate button so that
    ...
    * it recalcuates and re-diplay of the Simpson's Index
...
Let's implement push notifications for when the biodiversity index drops below 0.7. Or more specifically
...
* when simulation ends, add a restart button along the bottom row of buttons
* the restart button resets the sim so that:
    * the computer generates a new score
    * the player starts with 5 unpinned rows of forest
    * start and Pin are disabled, but generate button is enabled

(5 points) __Increase size of forest__

* instead of an 8x8 forest, make it something bigger
